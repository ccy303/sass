<template>
    <base-page :padding="20">
        <base-waterfall ref="Waterfall" :column="2">
            <template #default="{ list }">
                <base-waterfall-column v-for="(item, index) in list" :key="index">
                    <view class="goods-item" v-for="(item2, index2) in item" :key="index2">
                        <img :src="item2.url" mode="widthFix" class="img" />
                        <base-text :value="item2.title" :margin="20" :line-height="1.5" />
                    </view>
                </base-waterfall-column>
            </template>
        </base-waterfall>

        <base-loadmore :loading="loading" :divider="false"></base-loadmore>
    </base-page>
</template>
...
<script setup>
    ...
    import { ref } from "vue";
    import { usePager } from "@/gxota/hooks";
